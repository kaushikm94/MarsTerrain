...
//	ofSetWindowShape(1024, 768);
	...
	TreeNode root;
	createOctree(mars.getMesh(0), root, 8);
}
...
//	cout << ofGetFrameRate() << endl;
	...
	float hue = 0, sat = 255, light = 100;

	srand(time(0));
	hue = (rand() % 255) + 1;
	light+=50;
	ofColor boxColor = ofColor(0);
	boxColor.setHsb(hue, sat, light);
...
void ofApp::createOctree(const ofMesh& mesh, TreeNode& root, int lvl){
	cout << "inside Create octree" << endl;
...
//		root.setChild(tNode);
//	}
	subdivide(mesh, root, 8);

//	cout << "Root:" << root.points;

}

void ofApp::subdivide(const ofMesh& mesh, TreeNode& node, int lvl){
	cout << "Inside subdivide" <<" Level:" << lvl << endl;
	if (lvl == 0){
		cout << "Exiting subdivide" << endl;
		return;
	}
	int childCount = 0;
	vector<Box> boxList;
	subDivideBox8(node.box , boxList);
    cout << "boxlist size:" << boxList.size() << endl;
	for(int i=0; i < boxList.size(); ++i){
		vector<int> boxPoints;
		getMeshPointsInBox(mesh, node.points, boxList[i], boxPoints);
		if(boxPoints.size() > 0){
			TreeNode tNode;
			tNode.setBox(boxList[i]);
			tNode.setPoints(boxPoints);
		    node.setChild(tNode);
		    childCount++;
		}
		cout << "for box:" << i << endl;
	...
	lvl -= 1;
	cout << "Level:" << lvl << endl;
	cout << "N of children:" << childCount << endl;
	for(int i=0; i < childCount; ++i){
		cout << "Calling for " << i <<"th" << "child" << endl;
		subdivide(mesh, node.children[i], lvl);
	}
